...
def analyze_javascript(domain: str, output_dir: Path) -> None:
    """Analyze JavaScript files using LinkFinder to discover endpoints.
    ...
    logger.info(f"Running LinkFinder JavaScript analysis for {domain} on {len(urls)} URLs...")

    # Output file for consolidated LinkFinder results
    linkfinder_output = javascript_dir / "linkfinder_results.txt"

    # Process each URL with LinkFinder
    all_results = []
    for url in urls:
        try:
            logger.debug(f"Analyzing JavaScript for URL: {url}")

            # Run LinkFinder command for each URL
            linkfinder_cmd = [
